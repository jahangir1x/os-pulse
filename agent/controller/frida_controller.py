...
class FridaController:
    ...
    def send_ping(self) -> None:
        ...
        if self.script:
            try:
                self.script.post({'type': 'ping'})
                print(f"{Fore.CYAN}[PING] Sent ping to agent")
            except Exception as e:
                print(f"{Fore.RED}[ERROR] Failed to send ping: {e}")
    
    def request_status(self) -> None:
        """Request status from the agent"""
        if self.script:
            try:
                self.script.post({'type': 'status'})
                print(f"{Fore.CYAN}[STATUS] Requested status from agent")
            except Exception as e:
                print(f"{Fore.RED}[ERROR] Failed to request status: {e}")
